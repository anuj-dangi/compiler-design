	...
	for(int i=0;i<n;i++)
	{
		if(prod[i][0] == nt[index])
		{
			...
			for(int j=0; firstRHS[j] != '\0'; j++)
			{
				...
				for(int l=0; prod[i][l] != '\0'; l++)
				{
					table[index][indexOf(firstRHS[j])][l] = prod[i][l];
					table[index][indexOf(firstRHS[j])][l+1] = '\0';
				}
			}
			if(isPresent(firstRHS, '~'))
			{
				for(int j=0; follow[index][j] != '\0'; j++)
				{
					...
					for(int l=0; prod[i][l] != '\0'; l++)
					{
						table[index][indexOf(follow[index][j])][l] = prod[i][l];
						table[index][indexOf(follow[index][j])][l+1] = '\0';
					}
				}
			}
		}
